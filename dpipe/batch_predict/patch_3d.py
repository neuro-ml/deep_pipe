...
def divide_x(x, x_intersection_sizes: np.array, x_patch_sizes: np.array,
             padding_values):
    xs_parts = []
    for x_intersection_size, x_patch_size in zip(x_intersection_sizes,
                                                 x_patch_sizes):
        x_parts = divide_spatial(
...
class Patch3DPredictor(BatchPredict):
    def __init__(self, x_spatial_patch_sizes: list, y_spatial_patch_size: list,
        ...
        assert self.x_spatial_patch_sizes.shape[1] == \
            len(self.y_spatial_patch_size) == 3
        np.testing.assert_equal(np.unique(self.x_spatial_patch_sizes % 2),
                                np.unique(self.y_spatial_patch_size % 2))
    ...
    def validate(self, x, y, *, validate_fn):
        xs_parts = divide_x(x, self.x_spatial_intersection_sizes,
                            self.x_spatial_patch_sizes,
                            x.min(axis=spatial_dims, keepdims=True))
        y_parts_true = divide_y(y, self.y_spatial_patch_size)

        y_preds, loss = validate_object(zip(*xs_parts, y_parts_true),
