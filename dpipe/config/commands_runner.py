...
def load_or_create(load: Callable, create: Callable, path: str):
    """
    ``load`` the object located at ``path`` if it exists,  otherwise ``create`` it.

    Parameters
    ----------
    load: Callable(path)
    create: Callable(path)
        saves an object to ``path`` and returns it.
    path: str
    """
