...
def build_model(t_det_in, t_con_in, t_coord, t_ftr_in, kernel_size, n_classes, training, name, *,
    ...
    with tf.variable_scope(name):
        ...
        t_com = tf.concat([t_con, t_det, t_coord, t_feature], axis=1)
...
@register('deepmedic_feat_coordinates')
class DeepMedicEls(ModelCore):
    ...
    def build(self, training_ph):
        ...
        x_coordinates_ph = tf.placeholder(tf.float32, (None, 3, None, None, None), name='x_coord')
        ...
        logits = build_model(x_det_ph, x_con, x_coordinates_ph, x_feature, self.kernel_size, self.n_chans_out,
                             training_ph, name='deep_medic', dropout=dropout)

        return [x_det_ph, x_con_ph, x_coordinates_ph, x_ftr_ph], logits
