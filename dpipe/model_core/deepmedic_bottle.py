...
# Residual Block
def bottleneck(t, n_chans, kernel_size, training, name):
    ...
    with tf.variable_scope(name):
        with tf.variable_scope('transform'):
            ...
            if n_chans_dif != 0:
                t2 = cb(t2, n_chans, 1, training, 'cb')

        t1 = t
        t1 = cba(t1, n_chans//4, 1, training, 'encode')
        t1 = cba(t1, n_chans//4, kernel_size, training, 'process')
        t1 = cb(t1, n_chans, 1, training, 'decode')
...
def build_path(t, blocks, kernel_size, training, name):
    with tf.variable_scope(name):
        for i, n_chans in enumerate(blocks):
            t = bottleneck(t, n_chans, kernel_size, training, f'Bottleneck_{i}')

        return t


def build_model(t_det_in, t_con_in, kernel_size, n_classes, training, name,
                path_blocks=(32, 32, 32, 64), n_chans_common=128):
    with tf.variable_scope(name):
        ...
        t_highway = t_det_in
        s = (kernel_size//2) * 1 * len(path_blocks)
        with tf.variable_scope('highway'):
            t_highway = t_highway[:, :, s:-s, s:-s, s:-s]
            t_highway = spatial_batch_norm(t_highway , training=training,
                                           data_format='channels_first')

        t_comm = tf.concat([t_highway, t_det, t_con_up], axis=1)

        t_comm = cba(t_comm, n_chans_common, 1, training, name='comm_1')
        t_comm = cba(t_comm, n_chans_common, 1, training, name='comm_2')
        return cb(t_comm, n_classes, 1, training, 'C')


class DeepMedicBottle(ModelCore):
