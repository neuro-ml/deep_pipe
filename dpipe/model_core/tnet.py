...
def get_upsample_op(name, kernel_size):
    if name == 'neighbour':
        return nn.Upsample(scale_factor=kernel_size, mode='bilinear')
    else:
        raise ValueError(f"unknown upsample op name: {name}")

class TNet(nn.Module):
    def __init__(self, n_chans_in, n_chans_out, kernel_size=3, *, activation, structure, stride, conv_block):
        super().__init__()
