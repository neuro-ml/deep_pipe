...
def _make_tnet_with_dropout(conv_block, conv_transposed_block):
    class TnetWithDropout(nn.Module):
        def __init__(self, n_chans_in, n_chans_out, activation, structure, stride, p=0.5):
            super().__init__()
        ...
        def forward(self, input):
            ...
            for bridge_output, up_step, up_level in reversed(list(zip(bridge_outputs[:-1], self.up_steps,
                ...
                bottom_input = dropout(bottom_input, p=self.p, training=True)

            return self.output_layer(bottom_input)

    return TnetWithDropout


TNet2d_with_dropout = _make_tnet_with_dropout(ConvBlock2d, ConvTransposeBlock2d)
TNet3d_with_dropout = _make_tnet_with_dropout(ConvBlock3d, ConvTransposeBlock3d)

register('tnet2d_with_dropout')(TNet2d_with_dropout)
register('tnet3d_with_dropout')(TNet3d_with_dropout)
